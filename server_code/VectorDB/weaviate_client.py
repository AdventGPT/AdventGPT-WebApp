...
class WeaviateWCS:
    ...
    model_name: str= "OpenID model name for embeddings"
    ...
    def __init__(self, 
                 ...
                 embedded: bool=False,
                 openai_api_key: str=None,
                 dimensions: int=1024*3,
                 return_properties: list[str]=['content', 'doc_id', "book", "chapter_title", "chapter_number", "sentence_number"],  
                 **kwargs
                ):
        ...
        assert self.model_name in self.OPENAI_EMBEDDING_MODELS , 'Model not supported'
        
        if not openai_api_key:
            raise ValueError(f'OpenAI API key must be provided to use this model: {self.model_name}')
    ...
    def vector_search(self,
        ...
        self._connect()
        return_properties = return_properties if return_properties else self.return_properties
        query_vector = self._create_query_vector(request)
        collection = self._client.collections.get(collection_name)
    ...
    def get_openai_embedding(self, query: str) -> list[float]:
        ...
        '''
        embedding = self.model.embeddings.create(input=query, model=self.model_name).model_dump()
        if embedding:
